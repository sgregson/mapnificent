...
(function () {
  ...
  function getProgressBar(percent) {
    return $(
      '<div class="progress">' +
      '<div class="progress-bar progress-bar-mapnificent"  role="progressbar" aria-valuenow="' +
      percent +
  ...
  MapnificentPosition.prototype.init = function () {
    ...
    this.marker.bindPopup(this.popup).addTo(this.mapnificent.map);
  ...
  MapnificentPosition.prototype.renderProgress = function () {
    ...
    var removeSpan = $(
      '<span class="position-remove glyphicon glyphicon-trash pull-right">'
    ).on("click", function () {
  ...
  MapnificentPosition.prototype.updateControls = function () {
    ...
    var input = $('<input type="range">')
      .attr({
        max: Math.round(
          this.mapnificent.settings.options.maxWalkTravelTime / 60
        ),
        min: 0,
        value: minutesTime,
      })
