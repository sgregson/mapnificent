var Quadtree = (function () {
  ...
  Quadtree.prototype.subDivide = function () {
    ...
    for (var i = 0; i < this.points.length; i += 1) {
      ...
      console.warn(
        "Quadtree: Subdivide - could not insert point",
        this.points[i]
      );
    }
    this.points = null;
  ...
  Quadtree.prototype.searchInRadius = function (lat, lng, radius) {
    ...
    var x1 =
      lng -
      (180.0 / Math.PI) *
      (radius / earthRadius / Math.cos((lat * Math.PI) / 180.0));
    var x2 =
      lng +
      (180.0 / Math.PI) *
      (radius / earthRadius / Math.cos((lat * Math.PI) / 180.0));
  ...
  Quadtree.prototype.distanceBetweenCoordinates = function (
    lat,
    lng,
    slat,
    slng
    ...
    return (
      Math.acos(
        ...
        Math.cos(slat * toRad) *
        Math.cos(lat * toRad) *
        Math.cos((lng - slng) * toRad)
      ) * EARTH_RADIUS
    );
  };

  Quadtree.prototype.getDistancesInRadius = function (
    lat,
    lng,
    radius,
    sorted
    ...
    for (var i = 0; i < stops.length; i += 1) {
      results.push([
        stops[i],
        this.distanceBetweenCoordinates(lat, lng, stops[i].lat, stops[i].lng),
      ]);
    }
    if (sorted) {
